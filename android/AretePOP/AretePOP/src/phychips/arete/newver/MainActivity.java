...
public class MainActivity extends Activity implements iRcpEvent
{
	...
	startAutoRead.setOnClickListener(new OnClickListener()
	{
		...
		if (RcpApi.isOpen)
		{
		    ...
		    Toast.makeText(MainActivity.this, "Start update the data to server.",Toast.LENGTH_SHORT).show();
		    MainActivity.this.firstReset();
		}
		else
		{
    ...
    {
	{
		...
		if (newTagReceived)
		{
		    ...
		    uploadData(ttag.getName());
		    runOnUiThread(new Runnable()
		    {
			public void run()
			{
			    Toast.makeText(MainActivity.this, ttag.getName(),Toast.LENGTH_SHORT).show();
			    tagArray.add(ttag);
			    tagAdapter.notifyDataSetChanged();
    ...
    private void firstReset(){
	 // Create a new HttpClient and Post Header
	    ...
	    HttpPost httppost = new HttpPost("http://rfid.attocode.com/backend.php");
    ...
    private void uploadData(String rfid){
	 // Create a new HttpClient and Post Header
	    HttpClient httpclient = new DefaultHttpClient();
	    HttpPost httppost = new HttpPost("http://rfid.attocode.com/backend.php");
	    ...
	    try {
	        // Add your data
	        List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(2);
	        nameValuePairs.add(new BasicNameValuePair("json", rfid));
	        nameValuePairs.add(new BasicNameValuePair("password", "dsalkhzkjchaijstdgwquyhyedtwak;ldjsakdhsalkjd"));
	        httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs));
